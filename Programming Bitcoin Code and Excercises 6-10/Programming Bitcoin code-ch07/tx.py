...
class Tx:
    ...
    def sig_hash(self, input_index):
        ...
        result = int_to_little_endian(raw_version, 4)
    ...
    def verify_input(self, input_index):
        ...
        inpt = self.tx_ins[input_index]
        # grab the previous ScriptPubKey
        prev_scriptpubkey = inpt.script_pubkey(self.testnet)
        # get the signature hash (z)
        sighash = self.sig_hash(input_index)
        # combine the current ScriptSig and the previous ScriptPubKey
        combined_script = inpt.script_sig + prev_scriptpubkey
        # evaluate the combined script
        return combined_script.evaluate(sighash)
